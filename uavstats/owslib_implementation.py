...
class WebProcessingService(object):
    ...
    def describeprocess(self, identifier, xml=None):
        ...
        for process in processes:
            if process.identifier == identifier:
                return process
        raise ValueError(
            'process with identifier {} not found'.format(identifier))
    ...
    def execute(self, identifier, inputs, output=None, mode=ASYNC, lineage=False, request=None, response=None):
        ...
        if request is None:
            requestElement = execution.buildRequest(
                identifier, inputs, output, mode=mode, lineage=lineage)
...
class WPSExecuteReader(WPSReader):
    ...
    def __init__(self, timeout=None, auth=None, language=None):
        # superclass initializer
        super(WPSExecuteReader, self).__init__(
        ...
        if self.isSucceeded():
            ...
            if self.processOutputs:
                if identifier:
                    ...
                    outputs = [
                        o for o in self.processOutputs if o.identifier == identifier]
                    if outputs:
                        output = outputs[0]
                else:
...
class ComplexData(object):
    ...
    def __init__(self, mimeType=None, encoding=None, schema=None, maximumMegaBytes=None):
        ...
        self.schema = schema
        # defined only if provided by ProcessDescription ComplexData
...
class InputOutput(object):
            ...
            for sub_element in literal_data_element:
                ...
                if sub_element.tag.endswith('DataType'):
                    ...
                    if not self.dataType:
                        reference = sub_element.get(
                            nspath("reference", ns=subns))
    ...
    def _parseComplexData(self, element, complexDataElementName):
        ...
        if complex_data_element is not None:
            ...
            default_format_element = complex_data_element.find(
                'Default/Format')
...
class Output(InputOutput):
    ...
    def __init__(self, outputElement):
        ...
        if dataElement is not None:
            ...
            bboxDataElement = dataElement.find(
                nspath('BoundingBoxData', ns=namespaces['wps']))
    ...
    def writeToDisk(self, path=None, username=None, password=None,
        ...
        content = self.retrieveData(
            username, password, headers=headers, verify=verify, cert=cert)
...
class Process(object):
    ...
    def __init__(self, elem):
        ...
        self.processVersion = elem.get(
            nspath('processVersion', ns=wpsns)) or elem.get(nspath('processVersion'))
...
class ComplexDataInput(IComplexDataInput, ComplexData):
    ...
    def complexDataAsReference(self):
        ...
        refElement = etree.Element(nspath_eval(
            'wps:Reference', namespaces), attrib)
        return refElement
